...
export default function KeywordsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const pageList = Object.keys(pageMap).map(path => ({
        path,
        keywords: pageMap[path],
      }))

      // Add default homepage if no pages yet
      if (pageList.length === 0) {
        pageList.push({ path: '/', keywords: [] })
      }
      ...
      // Get site URL from latest audit
      const auditRes = await fetch('/api/audit')
      const auditData = await auditRes.json()
      if (auditData.reports?.[0]?.url) {
        setSiteUrl(auditData.reports[0].url)
      }
    }
    load()
  }, [])
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: '12px', alignItems: 'start' }}>
            ...
            {pages.map(p => (
              <div
                ...
                style={{
                  ...
                  border: `1px solid ${selectedPage === p.path ? 'rgba(30,144,255,0.3)' : 'rgba(0,0,0,0.08)'}`,
                  background: selectedPage === p.path ? 'rgba(30,144,255,0.06)' : '#fff',
        ...
        <div>
          <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '1.25rem', marginBottom: '12px' }}>
            <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '15px', fontWeight: 600, marginBottom: '4px' }}>
              {selectedPage}
            </div>
            ...
            <div style={{
              ...
              background: '#f8f9fb', minHeight: '42px', marginBottom: '0.75rem',
            }}>
              {keywords.map(kw => (
                <span key={kw} style={{
                  display: 'inline-flex', alignItems: 'center', gap: '4px',
                  background: 'rgba(30,144,255,0.1)', border: '1px solid rgba(30,144,255,0.2)',
                  color: '#1e90ff', fontSize: '12px', padding: '2px 8px', borderRadius: '20px',
                  fontFamily: 'Roboto Mono, monospace',
                }}>
                  {kw}
                  <span onClick={() => removeKeyword(kw)} style={{ cursor: 'pointer', fontSize: '14px', opacity: 0.6, lineHeight: 1 }}>×</span>
                </span>
              ))}
              <input
                type="text"
                ...
                style={{ background: 'transparent', border: 'none', outline: 'none', fontSize: '13px', color: '#0d1b2e', minWidth: '180px', flex: 1, fontFamily: 'Open Sans, sans-serif' }}
              />
            </div>

            <div style={{ display: 'flex', gap: '8px' }}>
              <button className="btn btn-ghost" onClick={saveKeywords} disabled={saving} style={{ fontSize: '13px' }}>
                {saved ? '✓ Saved' : saving ? 'Saving...' : 'Save Keywords'}
              </button>
              <button className="btn btn-accent" onClick={analyze} disabled={analyzing || !keywords.length} style={{ fontSize: '13px' }}>
          ...
          {analyzing && (
            <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '2rem', textAlign: 'center', color: '#7a8fa8', fontSize: '13px', fontFamily: 'Roboto Mono, monospace' }}>
          ...
          {analysis && (
            <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '1.25rem' }}>
              ...
              {/* Fixes */}
              <div style={{ fontSize: '11px', color: '#7a8fa8', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'Roboto Mono, monospace', marginBottom: '0.6rem' }}>What to fix</div>
                ...
                {analysis.fixes?.map((fix: any) => (
                  <div key={fix.priority} style={{ display: 'flex', gap: '10px', padding: '0.75rem 0', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                    <div style={{
                      ...
                      background: 'rgba(30,144,255,0.1)', border: '1px solid rgba(30,144,255,0.2)',
                      color: '#1e90ff', fontSize: '11px', display: 'flex', alignItems: 'center',
