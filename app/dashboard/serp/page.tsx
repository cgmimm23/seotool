      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '1.25rem', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', gap: '8px' }}>
          <input
            ...
            onKeyDown={e => e.key === 'Enter' && fetchSerp()}
            style={{ flex: 1 }}
          />
          <button className="btn btn-accent" onClick={fetchSerp} disabled={loading}>
            {loading ? 'Fetching...' : 'Check SERP'}
          </button>
        </div>
      </div>
        ...
        <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '1.25rem' }}>
          ...
          {results.map((r: any, i: number) => (
            <div key={i} style={{ display: 'flex', gap: '12px', padding: '0.85rem 0', borderBottom: i < results.length - 1 ? '1px solid rgba(0,0,0,0.06)' : 'none', alignItems: 'flex-start' }}>
              <div style={{ width: '28px', height: '28px', borderRadius: '6px', background: '#f0f4f8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 600, fontFamily: 'Roboto Mono, monospace', flexShrink: 0, marginTop: '2px', color: '#4a6080' }}>
                {r.position || i + 1}
              </div>
