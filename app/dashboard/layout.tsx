...
export default function SiteLayout({ children }: { children: React.ReactNode }) {
  ...
  // Extract site ID from pathname /dashboard/sites/[id]
  const siteId = pathname?.split('/dashboard/sites/')[1]?.split('/')[0]
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data } = await supabase.from('sites').select('id, name, url').eq('id', siteId).single()
  ...
  const navItems = [
    { label: 'Site Overview', href: `/dashboard/sites/${id}`, section: null },
      ...
      <aside style={{ width: '220px', flexShrink: 0, background: '#0d1b2e', display: 'flex', flexDirection: 'column', position: 'fixed', top: 0, left: 0, height: '100vh', zIndex: 50, overflowY: 'auto' }}>
        ...
        <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <a href="/dashboard" style={{ display: 'flex', alignItems: 'center', gap: '6px', textDecoration: 'none', marginBottom: '10px' }}>
            <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)', fontFamily: 'Roboto Mono, monospace' }}>All Sites</span>
          </a>
          {site ? (
            <>
              ...
              <a href={site.url} target="_blank" style={{ fontFamily: 'Roboto Mono, monospace', fontSize: '10px', color: '#1e90ff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block', textDecoration: 'none' }}>{site.url?.replace(/^https?:\/\//, '')}</a>
            </>
          ) : (
          ...
          {navItems.map(item => {
            ...
            return (
              <div key={item.label}>
                ...
                <a href={item.href} style={{ display: 'flex', alignItems: 'center', padding: '0.5rem 1rem', margin: '1px 0.5rem', borderRadius: '8px', fontSize: '13px', color: active ? '#fff' : 'rgba(255,255,255,0.6)', background: active ? 'rgba(30,144,255,0.3)' : 'transparent', fontWeight: active ? 600 : 400, textDecoration: 'none' }}>
                  {item.label}
                </a>
              </div>
            )
        ...
        <div style={{ padding: '1rem', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginBottom: '8px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'Roboto Mono, monospace' }}>{email}</div>
