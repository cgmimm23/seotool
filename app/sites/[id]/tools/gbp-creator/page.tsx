...
export default function GBPCreatorPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div>
      ...
      {step === 5 && (
        <div style={card}>
          <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '16px', fontWeight: 700, marginBottom: '4px' }}>Create Your Listing</div>
          <p style={{ fontSize: '13px', color: '#7a8fa8', marginBottom: '1.25rem' }}>Copy each field and paste into Google. Opens business.google.com in a new tab.</p>
          ...
          <ManualSubmit
            fields={[
              ...
              { label: 'Street Address', value: bizAddress },
              { label: 'City', value: bizCity },
              { label: 'State', value: bizState },
              { label: 'ZIP', value: bizZip },
              { label: 'Description', value: description },
              ...(isServiceAreaBiz && serviceAreas ? [{ label: 'Service Areas', value: serviceAreas }] : []),
            ]}
          />

          <div style={{ marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px dashed rgba(0,0,0,0.1)' }}>
            <div style={{ fontSize: '11px', color: '#7a8fa8', textTransform: 'uppercase' as const, letterSpacing: '0.06em', fontFamily: 'Roboto Mono, monospace', marginBottom: '8px' }}>Or submit via API (requires Google approval)</div>
          {!connected ? (
            <div style={{ padding: '0.5rem 0' }}>
              <button onClick={connectGoogle} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#fff', border: '1px solid rgba(0,0,0,0.15)', borderRadius: '6px', padding: '0.5rem 1rem', fontSize: '12px', cursor: 'pointer' }}>
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={submitToGoogle} disabled={submitting || !selectedAccount} style={{ background: 'transparent', border: '1px solid rgba(35,103,160,0.3)', borderRadius: '50px', padding: '0.4rem 1rem', fontSize: '12px', color: '#2367a0', cursor: selectedAccount ? 'pointer' : 'not-allowed', opacity: selectedAccount ? 1 : 0.5, fontWeight: 600 }}>
                  {submitting ? 'Submitting...' : 'Submit via API'}
                </button>
              </div>
            </>
          )}
...
function ManualSubmit({ fields }: { fields: { label: string; value: string }[] }) {
  ...
  async function copyAll() {
    const block = fields.filter(f => f.value).map(f => `${f.label}: ${f.value}`).join('\n')
    try {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap' }}>
        <a href="https://business.google.com/create" target="_blank" rel="noopener" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#2367a0', color: '#fff', borderRadius: '50px', padding: '0.55rem 1.25rem', fontSize: '13px', fontWeight: 700, textDecoration: 'none' }}>
          Open business.google.com →
        </a>
        <button onClick={copyAll} style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.15)', borderRadius: '50px', padding: '0.5rem 1rem', fontSize: '12px', cursor: 'pointer', fontWeight: 600, color: '#4a6080' }}>
          {copied === '__all__' ? '✓ All copied' : 'Copy all fields'}
        </button>
      </div>

      <div style={{ background: '#f8f9fb', border: '1px solid rgba(0,0,0,0.06)', borderRadius: '10px', padding: '0.5rem' }}>
        {fields.map(f => (
          <div key={f.label} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '10px 12px', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '10px', color: '#7a8fa8', textTransform: 'uppercase' as const, letterSpacing: '0.06em', fontFamily: 'Roboto Mono, monospace', marginBottom: '3px' }}>{f.label}</div>
              <div style={{ fontSize: '13px', color: f.value ? '#0d1b2e' : '#c0c0c0', fontWeight: 500, wordBreak: 'break-word' as const }}>{f.value || 'Not set'}</div>
            </div>
            <button
              onClick={() => copy(f.label, f.value)}
              disabled={!f.value}
              style={{ background: 'transparent', border: '1px solid rgba(0,0,0,0.12)', borderRadius: '6px', padding: '0.35rem 0.7rem', fontSize: '11px', color: '#4a6080', cursor: f.value ? 'pointer' : 'not-allowed', opacity: f.value ? 1 : 0.4, flexShrink: 0, fontWeight: 600 }}
            >
              {copied === f.label ? '✓' : 'Copy'}
            </button>
          </div>
