...
export default function GoogleAdsPage({ params }: { params: { id: string } }) {
  ...
  async function checkConnection() {
    ...
    try {
      const res = await fetch(`/api/google-ads/status?siteId=${params.id}`)
  ...
  async function disconnect() {
    if (!confirm('Disconnect Google Ads from this site? You will need to reconnect to load data.')) return
  ...
  async function fetchData() {
    ...
    setError('')
    try {
      const res = await fetch(`/api/google-ads?customerId=${customerId}&days=${days}&siteId=${params.id}`)
      const json = await res.json()
  ...
  if (!status?.connected) return (
    <div>
      ...
      <div style={{ ...card, textAlign: 'center', padding: '3rem' }}>
        ...
        <p style={{ fontSize: '14px', color: '#7a8fa8', marginBottom: '0.5rem', maxWidth: '480px', margin: '0 auto 0.5rem' }}>Connect your Google account to see campaign performance, keyword bids, quality scores, search terms, budget pacing, and ROAS for this site.</p>
        {status?.email && (
    ...
    <div>
      ...
      {accounts.length === 0 && (
        <div style={{ fontSize: '12px', color: '#7a8fa8', marginBottom: '1rem', padding: '8px 12px', background: 'rgba(255,165,0,0.05)', border: '1px solid rgba(255,165,0,0.2)', borderRadius: '8px' }}>
          No Google Ads accounts found for this Google login. Enter a Customer ID manually, or reconnect with a Google account that has access.
        </div>
      )}
