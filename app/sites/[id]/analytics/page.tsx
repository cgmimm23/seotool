...
export default function AnalyticsPage({ params }: { params: { id: string } }) {
  ...
  async function loadProperties() {
    ...
    try {
      const res = await fetch(`/api/analytics?action=list_properties&siteId=${params.id}`)
  ...
  async function fetchData() {
    ...
    setError('')
    try {
      const res = await fetch(`/api/analytics?siteId=${params.id}&propertyId=${propertyId}&days=${days}`)
      const json = await res.json()
  ...
  if (!connected) return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ fontSize: '13px', color: '#7a8fa8' }}>Site traffic and user behavior via Google Analytics 4</p>
      </div>
      <div style={{ ...card, textAlign: 'center', padding: '3rem' }}>
        <div style={{ fontFamily: 'Montserrat, sans-serif', fontSize: '18px', fontWeight: 700, marginBottom: '8px' }}>Connect Google for this site</div>
        <p style={{ fontSize: '14px', color: '#7a8fa8', marginBottom: '1.5rem', maxWidth: '460px', margin: '0 auto 1.5rem' }}>
          Sign in with the Google account that owns this website&apos;s Analytics. Each site gets its own connection, so you can use different Google accounts for different sites.
        </p>
  ...
  return (
    <div>
      ...
      {properties.length === 0 && !loadingProperties && (
        <div style={{ fontSize: '12px', color: '#7a5818', marginBottom: '1rem', padding: '8px 12px', background: 'rgba(228,179,79,0.08)', border: '1px solid rgba(228,179,79,0.25)', borderRadius: '8px' }}>
          No GA4 properties found on this Google account. Switch to a different account or enter the Property ID manually.
        </div>
      )}
