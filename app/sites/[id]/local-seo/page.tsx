...
export default function LocalSEOPage({ params }: { params: { id: string } }) {
  ...
  async function checkConn() {
    ...
    const { data: site } = await supabase.from('sites').select('google_access_token').eq('id', params.id).single()
    const s = site as any
  ...
  async function loadAccounts() {
    try {
      const res = await fetch(`/api/gbp/data?siteId=${params.id}`)
      const j = await res.json()
      if (j.error) throw new Error(j.error)
      setAccounts(j.accounts || [])
      const firstLoc = j.accounts?.[0]?.locations?.[0]
      if (firstLoc?.name) setSelectedLocation(firstLoc.name)
    } catch (e: any) { setGbpError(e.message) }
  }
        ...
        <>
          {checkingAuth ? (
            ...
            <div style={{ ...card, textAlign: 'center', padding: '3rem' }}>
              ...
              <button onClick={connect} style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', background: '#fff', border: '1px solid rgba(0,0,0,0.15)', borderRadius: '8px', padding: '0.65rem 1.25rem', fontSize: '13px', cursor: 'pointer' }}>
            ...
            <>
              <div style={{ ...card, display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                ...
                <select value={selectedLocation} onChange={e => setSelectedLocation(e.target.value)} className="form-input" style={{ flex: 1, minWidth: '260px', fontSize: '13px' }}>
