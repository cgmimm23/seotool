    ...
    <div>
      ...
      <div style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '12px', padding: '1.25rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '6px' }}>
            {(['all', 'dofollow', 'nofollow', 'toxic'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '12px', cursor: 'pointer', border: `1px solid ${filter === f ? '#1e90ff' : 'rgba(0,0,0,0.1)'}`, background: filter === f ? 'rgba(30,144,255,0.08)' : 'transparent', color: filter === f ? '#1e90ff' : '#7a8fa8', fontFamily: 'Open Sans, sans-serif' }}>{f}</button>
            ))}
          </div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 60px 80px 120px', gap: '12px', padding: '0.4rem 0.5rem', borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
          {['Domain', 'DA', 'Links', 'Type', 'Last Seen'].map(h => <div key={h} style={{ fontSize: '11px', color: '#7a8fa8', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'Roboto Mono, monospace' }}>{h}</div>)}
        </div>
        {filtered.map((b, i) => {
          ...
          return (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 60px 80px 120px', gap: '12px', alignItems: 'center', padding: '0.75rem 0.5rem', borderBottom: i < filtered.length - 1 ? '1px solid rgba(0,0,0,0.05)' : 'none' }}>
              <div><div style={{ fontSize: '13px', fontWeight: 600, color: '#0d1b2e' }}>{b.domain}</div></div>
              <div><span style={{ fontSize: '12px', fontFamily: 'Roboto Mono, monospace', fontWeight: 600, padding: '2px 10px', borderRadius: '20px', background: da.bg, color: da.color }}>{b.da}</span></div>
      ...
      <div style={{ marginTop: '12px', padding: '1rem', background: 'rgba(30,144,255,0.05)', border: '1px solid rgba(30,144,255,0.15)', borderRadius: '10px', fontSize: '13px', color: '#4a6080' }}>
        <strong style={{ color: '#1e90ff' }}>Coming soon:</strong> Live backlink data via a backlink API. Currently showing sample data.
      </div>
    </div>
  )
}
